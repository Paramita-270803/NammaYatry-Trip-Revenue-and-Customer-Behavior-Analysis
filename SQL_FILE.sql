...
-- Driver Utilization: List drivers who have completed more than 50 trips.
SELECT driverid, COUNT(*) AS trip_count FROM trips 
GROUP BY driverid HAVING trip_count > 50;

-- Single Trip High Spenders: Identify customers who have taken exactly 1 trip, but that trip cost more than 750.
SELECT custid, fare FROM trips WHERE custid IN (SELECT custid 
FROM trips GROUP BY custid HAVING COUNT(*) = 1) AND fare > 750;
